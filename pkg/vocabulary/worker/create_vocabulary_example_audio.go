...
func (w CreateVocabularyExampleAudioHandler) CreateVocabularyExampleAudio(ctx *appcontext.AppContext, payload domain.QueueCreateVocabularyExampleAudioPayload) error {
	ctx.Logger().Info("generate sound for vocabulary example", appcontext.Fields{"exampleID": payload.Example.ID, "content": payload.Example.FromLang})
	example := payload.Example

	result, err := w.ttsRepository.GenerateVocabularyExampleSound(ctx, example.ID, example.FromLang)
	if err != nil {
		ctx.Logger().Error("failed to generate sound for vocabulary example", err, appcontext.Fields{})
		return err
	}

	ctx.Logger().Text("set audio data")
	if err = example.SetAudio(result.FileName); err != nil {
		ctx.Logger().Error("failed to update audio to db document", err, appcontext.Fields{"fileName": result.FileName})
		return err
	}
