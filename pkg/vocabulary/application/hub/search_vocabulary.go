...
func (h SearchVocabularyHandler) SearchVocabulary(ctx *appcontext.AppContext, req *vocabularypb.SearchVocabularyRequest) (*vocabularypb.SearchVocabularyResponse, error) {
	...
	if vocabulary != nil {
		ctx.Logger().Text("vocabulary found in caching, respond")
		result.Found = true
		result.Vocabulary = dto.ConvertVocabularyFromDomainToGrpc(*vocabulary)
	...
	if vocabulary != nil {
		ctx.Logger().Text("vocabulary found in db")
